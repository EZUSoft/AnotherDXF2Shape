...
from qgis.PyQt.QtWidgets import QApplication, QMessageBox, QDialog, QTableWidgetItem, QFileDialog
from qgis.PyQt.QtCore import QSettings, QTranslator, QCoreApplication,Qt

try:

    from .qt_compat import (
        QT6, exec_dialog, DontResolveSymlinks, ShowDirsOnly, Close, MsgBox_Yes, MsgBox_No
    )
    from .fnc4all import *
...
if QT6:
    ...
    import encodings
    def getAvailableCharsets():
        common_codes = ['utf-8', 'utf-16', 'latin_1', 'cp1252', 'ascii']
        all_codes = sorted([enc for enc in set(encodings.aliases.aliases.values())
                            if enc not in common_codes])
        return ['System'] + common_codes + all_codes
else:
    ...
    def getAvailableCharsets():
        system_entry = ['System']
        common_codes = ['utf-8', 'utf-16', 'latin_1', 'cp1252', 'ascii']
        all_codes = sorted([str(c, 'utf-8') for c in QTextCodec.availableCodecs()
                           if str(c, 'utf-8') not in system_entry + common_codes])
        return system_entry + common_codes + all_codes
...
class uiADXF2Shape(QDialog, FORM_CLASS):
    def __init__(self, parent=None):
        ...
        locale_path = os.path.join(self.plugin_dir, 'i18n', 'translation_{}.qm'.format(locale))
    ...
    def FillPoint4Wld(self, wldname):
        ...
        if os.path.exists(wldname):
            ...
            if p1 != None:
                ...
                self.tabTPoints.item(0, 0).setText(str(p1[0][0]))
                self.tabTPoints.item(0, 1).setText(str(p1[0][1]))
                
                self.tabTPoints.item(0, 2).setText(str(p1[1][0]))
                self.tabTPoints.item(0, 3).setText(str(p1[1][1]))                
                if p2 != None:
                    self.tabTPoints.item(1, 0).setText(str(p2[0][0]))
                    self.tabTPoints.item(1, 1).setText(str(p2[0][1]))
                    self.tabTPoints.item(1, 2).setText(str(p2[1][0]))
                    self.tabTPoints.item(1, 3).setText(str(p2[1][1]))    
        else:
    ...
    def btnPastePoints_clicked(self):
        ...
        try:
            ...
            for line in lines:
                ...
                try:
                    coords = [float(part.replace(",", ".")) for part in parts] 
            ...
            if len(points) == 1:
                self.cbTArt.setCurrentIndex(0) 
            elif len(points) == 2:
                self.cbTArt.setCurrentIndex(1) 
            elif len(points) == 3:
                self.cbTArt.setCurrentIndex(2) 
            elif len(points) == 4:
                self.cbTArt.setCurrentIndex(3) 
    ...
    def SetzeVoreinstellungen(self):
        ...
        self.chkRawCode.setChecked(True if s.value("bRawCode", "Nein") == "Ja" else False)
        self.chkCol.setChecked(True if s.value("bGenCol", "Nein") == "Ja" else False)
        self.chkLay.setChecked(True if s.value("bGenLay", "Ja") == "Ja" else False)
        self.chkUseTextFormat.setChecked(True if s.value("bFormatText", "Ja") == "Ja" else False)
        self.chkUseColor4Point.setChecked(True if s.value("bUseColor4Point", "Ja") == "Ja" else False)
        self.chkUseColor4Line.setChecked(True if s.value("bUseColor4Line", "Ja") == "Ja" else False)
        self.chkUseColor4Poly.setChecked(True if s.value("bUseColor4Poly", "Nein") == "Ja" else False)
        self.chk3D.setChecked(True if s.value("bGen3D", "Nein") == "Ja" else False)
        self.chk3D_clicked()
        self.chkSHP.setChecked(True if s.value("bGenSHP", "Nein") == "Ja" else False)
        self.chkSHP_clicked()
        self.chkGPKG.setVisible(True)
        self.chkGPKG.setChecked(True if s.value("bGenGPKG", "Nein") == "Ja" else False)
        self.chkGPKG_clicked()


        iCodePage = int(s.value("iCodePage", 0))


        self.cbCharSet.clear()
        self.cbCharSet.addItems(getAvailableCharsets())


        from qgis.PyQt.QtWidgets import QComboBox


        self.cbCharSet.setEditable(True)


        if QT6:

            self.cbCharSet.setInsertPolicy(QComboBox.InsertPolicy.NoInsert)
        else:

            self.cbCharSet.setInsertPolicy(QComboBox.NoInsert)


        self.cbCharSet.completer().setFilterMode(Qt.MatchFlag.MatchContains)



        if 0 <= iCodePage < self.cbCharSet.count():
            self.cbCharSet.setCurrentIndex(iCodePage)
        else:
            self.cbCharSet.setCurrentIndex(0)  
    ...
    def SHPorGPKG(self):
        ...
        if bGenSHP or bGenGPKG:
            ...
            self.lbOutput.setText(self.tr("Output path"))
        else:
    ...
    def browseZielPfadOrDatei_clicked(self):
        ...
        if not os.path.exists(lastSHPorGPKGDir):
            lastSHPorGPKGDir = os.getcwd()
        outDirName = QFileDialog.getExistingDirectory(
            self, "Open Directory", lastSHPorGPKGDir, DontResolveSymlinks | ShowDirsOnly)
        outDirName = outDirName.replace("\\", "/")
        self.txtZielPfad.setText(outDirName)
        if outDirName != "":
            s.setValue("lastSHPorGPKGDir", outDirName)

    def OptSpeichern(self):        
        s = QSettings("EZUSoft", fncProgKennung())
        s.setValue("bRawCode", "Ja" if self.chkRawCode.isChecked() == True else "Nein")
    ...
    def btnStart_clicked(self):
        if self.listDXFDatNam.count() == 0 or (self.listDXFDatNam.count() == 1 and self.listDXFDatNam.item(0).text() == self.listEmpty):
            QMessageBox.critical(None, self.tr("Please specify a DXF-File"), self.tr("DXF-file not selected")) 
            return

        for i in range(self.listDXFDatNam.count()):
            AktDat = self.listDXFDatNam.item(i).text()
            if not os.path.exists(AktDat):
                QMessageBox.critical(None, self.tr("DXF-file not found"), AktDat)
                return
        ...
        out = "GPKG" if (int(Haupt) >= 1 and int(Neben) >= 1) else "SHP"
        ...
        Antw = DXFImporter(self, out, self.listDXFDatNam, ZielPfad, 
                          self.chkSHP.isChecked() or self.chkGPKG.isChecked(), 
                          self.cbCharSet.currentText(), self.chkCol.isChecked(), 
                          self.chkLay.isChecked(), self.chkUseTextFormat.isChecked(), 
                          self.chkUseColor4Point.isChecked(), self.chkUseColor4Line.isChecked(), 
                          self.chkUseColor4Poly.isChecked(), dblFaktor, 
                          self.chkTransform.isChecked(), DreiPassPunkte, 
                          self.chk3D.isChecked(), self.txtErsatz4Tab.text(), self.chkRawCode.isChecked())
        CheckVersion (int(time.perf_counter() - start))
        self.FormRunning(False)
    ...
    def FormRunning(self,bRun):
        ...
        Anz(self.btnReset)
        Anz(self.cbCharSet)
        Anz(self.button_box.button(Close))
